...
export class AddUpdateProductComponent implements OnInit {
  ...
  saveProductList(): void {
    ...
    this.product.name = this.product.name + "1";
    this.product.description = this.product.description + "1";
    this.products.push(this.product);

    this.product.name = this.product.name + "2";
    this.product.description = this.product.description + "2";
    this.products.push(this.product);

    this.product.name = this.product.name + "3";
    this.product.description = this.product.description + "3";
    this.products.push(this.product);


    this.productRepository.postProducts(this.products).then( () => {
     // this.eventService.getNotifier<Product>(EventType.NewProductAdded).publish(this.products);
      this.goBack();
    });
  }
